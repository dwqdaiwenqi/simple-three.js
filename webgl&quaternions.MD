...
**ps**：简要概括下里面的推导过程，为了推出四元数旋转的矩阵,需要用到之前的**p'=qpq`*`** 把它写出矩阵的形式，其中需要一个q的左乘矩阵和一个q的右乘矩阵，可以通过pq和qp相乘结果获得，相乘的结果是个线性组合，因此可以用矩阵的形式表示粗来。所以p'=L(q)R(q`*`)p ,然后会通过a&sup2;+b&sup2;+c&sup2;+d&sup2;=1进行化简，直到最后的旋转四元数矩阵形式
