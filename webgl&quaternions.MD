...
#### 旋转模型
现在已经能使用四元数控制一个点（模型的世界坐标）的旋转，那么也能把四元数旋转应用到模型上，让模型朝着目标方向自旋转。这里构成旋转四元数的旋转弧度和旋转轴是如何获得的呢？通常会构建一个中间向量up，up为[0,1,0],旋转弧度可以利用up与目标方向进行点乘获得，旋转轴是利用目标方向与up的叉乘得到的，这样旋转四元数就形成了，最后再把这个旋转四元数转换为矩阵形式：
